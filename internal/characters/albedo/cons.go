	...
	return func() {
		...
		if inSkillArea {
			...
			active.AddAttackMod(character.AttackMod{
				Base: modifier.NewBaseWithHitlag("albedo-c4", 60),
...
func (c *char) c6(lastConstruct int) func() {
	return func() {
		...
		lunarCrystallize := c.Core.Constructs.CountByType(construct.GeoConstructLunarCrystallize) > 0
		inSkillArea := c.Core.Combat.Player().IsWithinArea(c.skillArea)
		if (crystallizeShield || lunarCrystallize) && inSkillArea {
			active := c.Core.Player.ActiveChar()
			m := make([]float64, attributes.EndStatType)
			m[attributes.DmgP] = 0.17
			active.AddStatMod(character.StatMod{
				Base:         modifier.NewBase("albedo-c6", 60),
